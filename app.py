...
def prediction_page():
    ...
    if predict_btn:
        ...
        risk_label = "Low Risk" if prob_positive < 30 else "Moderate Risk" if prob_positive < 70 else "High Risk"
        ...
        table_data = [[k, str(v)] for k, v in prediction_data["inputs"].items()]
        table = Table(table_data, colWidths=[2.5*inch, 3*inch])
        table.setStyle(TableStyle([("GRID", (0,0), (-1,-1), 1, colors.grey)]))

        elements.append(Paragraph("Patient Inputs", styles["Heading2"]))
        elements.append(table)
        elements.append(Spacer(1, 0.3 * inch))
        elements.append(Paragraph(f"<b>Result:</b> {risk_label}", styles["Normal"]))
        elements.append(Paragraph(f"<b>Probability:</b> {round(prob_positive,2)}%", styles["Normal"]))
